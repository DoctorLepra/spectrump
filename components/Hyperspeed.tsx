...
class App {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    ...
    const initW = Math.max(1, container.offsetWidth || 800);
    const initH = Math.max(1, container.offsetHeight || 600);
    ...
    try {
      ...
      // Ignore if context creation fails
    }
  ...
  initPasses() {
    try {
      ...
      // Fallback if postprocessing pass initialization encounters context limits
      console.warn("Hyperspeed postprocessing fallback enabled");
    }
  }
...
export const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  return <div id="lights" className="w-full h-full min-h-[400px]" ref={hyperspeed}></div>;
};
